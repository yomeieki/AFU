 ...
 * 解析。进程时区已由 `utils/timezone.ts` 钉死 Asia/Shanghai（不再依赖服务器 / PM2 的环境
 * TZ），但这条校验仍保留——不让「金额几时生效」这件事再依赖任何进程配置，哪怕将来钉死
 * 逻辑被改动，不带偏移的写法也会被这里挡住，而不是悄悄按当时的进程 TZ 解析。后台页的
 * joinIso 一律拼 `+08:00`，所以这条只拦直接调 API 的写法，不影响店主操作。
 */
