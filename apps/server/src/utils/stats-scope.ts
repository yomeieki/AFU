 ...
 *
 * 为什么抽成常量：接入点已经有 9 处（后台概览 3、工作台 3、趋势 1、扫码转化 1、按商品转化 1），
 ...
 *
 * 挡不住的地方有两处，都不是漏接，是这个口径本身够不到：
 *  1. 热销榜读 Product.salesCount 冗余列（下单瞬间 +1，与订单行无关），加多少 where 都过滤不掉。
 *     靠「联调用专门的测试商品、联调后软删除」隔离。
 *  2. 扫码转化率的**分母**（scan_logs 的扫码次数）没有测试标记，只有分子被过滤了。
 *     联调账号若同区间既扫码又下测试单，转化率会偏低。见 scan-stats.ts 该处注释。
 * 两处都详见 docs/ops-test-orders.md。
 */
