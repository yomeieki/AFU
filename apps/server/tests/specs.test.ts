...
function sku(id: number | undefined, specText: string, specValues: string[], sortOrder = 0) {
  return { ...(id !== undefined ? { id } : {}), specText, specValues, price: 100, originalPrice: null, stock: 1, sortOrder }
}
...
test('S4 两阶段同步：维度重排后两个 SKU 的 specText 互换不撞唯一索引', async () => {
  const { tx, getRows } = makeFakeTx([
    ...
    { id: 2, productId: 1, specText: 'B/X', specValues: ['B', 'X'], price: 200, originalPrice: null, stock: 2, sortOrder: 1 },
  ])
...
test('S5 维度重排：4 个 SKU 的 specText 两两互换，id 全部保留', async () => {
  ...
  for (const [id, specText, specValues] of [
    [1, 'x/a', ['x', 'a']],
    [2, 'y/a', ['y', 'a']],
    [3, 'x/b', ['x', 'b']],
    [4, 'y/b', ['y', 'b']],
...
test('S6 删除与新建：未携带的旧行删除，无 id 的行新建', async () => {
  ...
  await syncProductSkus(tx, 1, [sku(2, 'B', ['B'], 0), sku(undefined, 'A', ['A'], 1)])
  let rows = getRows()
  assert.equal(rows.length, 2)
  assert.ok(!rows.some((r) => r.id === 1), 'id1 应被删除')
  assert.ok(rows.some((r) => r.id === 2 && r.specText === 'B'))
