...
/** 暂停是否仍生效：until 为空 = 手动恢复；否则到时自动失效（与服务端 isPaused/isPickupPaused 同口径） */
export const pauseActive = (p: { until: string | null } | null | undefined, now = Date.now()): boolean =>
  !!p && (!p.until || Date.parse(p.until) > now)

/** 休业是否仍生效：until 为空 = 手动恢复；否则含当天仍休业、次日恢复（与服务端 isHolidayNow 同口径） */
export const holidayActive = (h: { until: string | null } | null | undefined, todayKeyStr: string): boolean =>
  !!h && (h.until === null || todayKeyStr <= h.until)

export function pauseStateLines(s: PauseState, now = Date.now(), today = todayKey()): { key: 'HOLIDAY' | 'DELIVERY' | 'PICKUP'; text: string }[] {
  const out: { key: 'HOLIDAY' | 'DELIVERY' | 'PICKUP'; text: string }[] = []
  if (holidayActive(s.holiday, today)) out.push({ key: 'HOLIDAY', text: `休业中：${s.holiday!.reason || '休业'}（${s.holiday!.until ? `${s.holiday!.until.slice(5)} 后恢复` : '手动恢复'}）` })
  if (pauseActive(s.paused, now)) out.push({ key: 'DELIVERY', text: `外送已暂停：${s.paused!.reason || '手动暂停'}` })
  if (s.pickupEnabled && pauseActive(s.pickupPaused, now)) out.push({ key: 'PICKUP', text: `自取已暂停：${s.pickupPaused!.reason || '手动暂停'}` })
  return out
}
