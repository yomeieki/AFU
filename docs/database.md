...
订单号格式：`ORD` + 日期(8位，Asia/Shanghai) + **打散后的当日号(4位)**

示例：`ORD202609077314`，小票与工作台显示为 `#7314`。
...
实现见 `apps/server/src/services/order-no.ts`，两层：

1. **唯一性**靠当日流水。计数器表 `order_no_seq` 一行/天，用
   `INSERT ... ON DUPLICATE KEY UPDATE seq = LAST_INSERT_ID(seq + 1)` 原子自增，
   在下单事务**之外**取号（放进去会把计数器行锁持有到整笔交易结束，下单被串行化）。
   下单失败会烧掉一个号、留下空档——这是给人读的号，不是会计流水。
2. **保密**靠 4 轮平衡 Feistel 置换。流水号不能直接印在票上：`#0087` 等于告诉顾客
   今天做了 87 单。Feistel 把 0000–9999 一一映射到 0000–9999（**双射**，所以唯一性
   原样保住），相邻流水映射出来的值毫无关联；密钥掺了日期，换一天整张映射表就变。

超过 9999 单/天时会多出一位（4 位 + 圈数），此时后四位才可能重复一次。
