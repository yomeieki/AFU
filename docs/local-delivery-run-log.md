...
### 结论 2c：重量 × 距离 × 运费（2026-09-08 向快递100 实测 16 次 batchPrice）

**背景**：顾客侧查价 `kd100Provider.price()` 里 `weightKg` 写死 **0.5**，而真正下单
（`batchOrder`）报的是**真实重量**。也就是「按 0.5 kg 报价收钱、按真实重量付钱」。
实测这个缺口有多大。

**最低报价矩阵**（同一时刻、同一组坐标，只变重量）：

| 距离 | 0.5 kg | 2 kg | 5 kg | 10 kg |
|---|---|---|---|---|
| 1.78 km | ¥6.05 | ¥6.05 | ¥6.05 | ¥6.05 |
| 3.62 km | ¥8.25 | ¥8.25 | ¥8.25 | ¥8.25 |
| 5.15 km | ¥11.55 | ¥11.55 | ¥11.55 | ¥11.55 |
| 9.53 km | ¥18.76 | ¥18.76 | ¥18.76 | **¥19.86** |

**① ≤5 kg 重量完全不参与计价。** 四家运力 × 四个距离，0.5/2/5 kg 报价一模一样。
按默认 300 g/件算，5 kg = 17 件；8 样菜的大单才 2.4 kg，日常订单碰不到这条线。

**② >5 kg 才收续重费，且是与距离无关的一口价：**
...
**③ 结论：目前敞口 ≤ ¥1.10，PO 2026-09-08 决定不修。**
因为只呼最低价那家，而**蜂鸟不收续重费**，它把最低价的天花板摁住了：
1.78/3.62/5.15 km 低估 ¥0.00，9.53 km 低估 ¥1.10。

> ⚠️ **这个「没事」完全依赖蜂鸟在运力表里。**
> 后台把蜂鸟摘掉 → >5 kg 的敞口跳到达达 +¥1.10；蜂鸟和达达都摘掉 → 顺丰 +¥8.80 /
> 闪送 +¥11.00，**翻十倍，且没有任何提示**。
> **改 `kd100.providers` 之前回来看这一段。** 真要修：`/local/quote` 收购物车项、
> 服务端自己算重量（与下单侧同一个算式）传给 `price()`——不能让客户端传重量。

**④ 顺带发现：7 家运力里只有 4 家真的会报价。** 后台配了顺丰/蜂鸟/美团/闪送/达达/
UU跑腿/光年达，但 16 次查价里**美团、UU跑腿、光年达一次都没出现**（大概率自贡未开通）。
影响：三级阶梯第三级「并呼全部」实际只呼得到 4 家，冻结额比估算低；那 3 家是死配置，
建议摘掉（少 3 次无效外呼）。
