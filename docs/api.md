...
**并发语义（2026-09-24 起）：** 事务内锁序固定为「本单涉及的 sku 行（按 skuId 升序、条件更新扣减）→ 本单涉及的商品行（按 productId 升序、条件更新扣减+加销量）→（本单用了积分时 `FOR UPDATE` 锁用户行）→ 插入 `orders`/`order_items`」——扣减挪到插入订单行之前，插行时外键校验要的商品行 S 锁被自己已持有的 X 锁覆盖，不再与自己或 `PUT /api/admin/products/:id/stock`、取消/退款回滚互相升级锁形成死锁（旧版本先插行再扣库存，两笔同商品并发下单会互相死锁并直接以 `50001` 报给顾客）。同一商品/规格的并发下单现在要么全部成功、要么以业务错误码 `42201`（库存不足）失败，不会再出现因锁序导致的 `50001`。事务整体还包一层重试：命中 MySQL 死锁（1213 → Prisma `P2034`，或 `$queryRaw` 撞上同一错误号时 Prisma 包装成的 `P2010`）时自动重试最多 5 次（间隔 50ms×次数 + 随机抖动）。与加购（`POST /api/cart`，2026-09-24 起已对齐为 L1(sku)→L2(product)→L3(user) 的共享锁定读，见 `routes/cart.ts`）、取消/退款回滚（`utils/order-stock.ts`）、改库存（`PUT /api/admin/products/:id/stock`）并发时，各方锁序两两同向，只会排队等待，不会成环；重试兜的是本方案未覆盖的残余环（如后台整包编辑商品按请求体顺序锁 sku）。重试与幂等键 `(userId, clientRequestId)` 的冲突处理互不影响，重试期间对调用方透明。**5 次重试后仍死锁：维持原样上抛**——`50001` + 企微「接口 500」告警（不改错误码语义），小程序 `utils/request.js` 已把 `50001` 映射为「系统开小差了，请稍后再试」；这样真出现结构性问题时店主能立刻收到通知，而不是被更友好的文案掩盖。详见 `docs/superpowers/plans/2026-09-24-order-deadlock.md`。
...
**并发语义（2026-09-24 修订 2；锁序分析 2026-09-24 修订 3 改正；两道防线的分工 2026-09-24 修订 4）：**
- **有规格**：事务内先 `SELECT ... FOR UPDATE` 锁定读该规格行的当前真实值（不是 REPEATABLE-READ 快照），把这个规格写成传入的绝对值，再用 `delta = 新值 − 旧值` 对 `product.stock` 做**相对增量**——不会覆盖并发下单扣减/取消回滚对其它规格的改动。锁序固定为「规格行 → 商品行」，与取消/退款回滚（`utils/order-stock.ts`，锁序同为规格行 → 商品行）一致；**2026-09-24 起下单事务（`routes/orders.ts`）改为先扣减（sku 行 X 锁 → 商品行 X 锁）再插 `order_items`，与本接口同序**，不再因为插行时的外键 S 锁与本接口的规格行 X 锁互相升级成环。本接口保留的 P2034 重试并未因此变成摆设：它是为「后台整包编辑商品」（`routes/admin/products.ts` 的 `PUT /:id`，sku 锁序按请求体顺序、暂未纳入本次全局锁序）与未来可能新增的路径兜底，本接口自身与另一次本接口调用之间仍可能因并发改同一商品的不同规格而触发普通锁等待（同向，不成环）。`FOR UPDATE` 与死锁重试各管一类交错：`FOR UPDATE` 防的是**快照陈旧**（对方先锁住这一个规格行、持锁到提交才放，如取消/退款回滚——这时 PUT 不持任何锁，不会死锁，全靠锁定读拿到真实值）；死锁重试防的是**锁序成环**（本接口与整包编辑商品等锁序未对齐的路径并发时）。e2e 覆盖分别见 `scripts/e2e.d/74-low-stock.sh` 的 74.11-g（快照陈旧形态）与 74.11-e（用 raw SQL 模拟旧形态复现锁序成环，验证本接口自身的 P2034 重试仍然有效）。
...
| `schedAutoCall` | `autoCallScheduled` | 已备好且 `callAt` 到；总开关关/熔断/呼叫失败（非 42204/42225）时按单发企微告警而不再静默跳过，返回值=成功呼叫数+告警数（2026-09-23 S4/S5；42204/42225 复核裁决 R8 追加排除——运力方失败/订单状态刚变化两类 callRider 内部已处理或属自愈，不重复告警） |
| `schedLate` | `remindScheduledLate` | `scheduledAt`+10 分未取餐，限频 60 分钟 |

`remindUnacceptedOrders / autoCallRiders / remindLocalUncalled / autoRejectStaleCancelRequests / refreshStaleQuotes` 排除预约单（`refreshStaleQuotes` 2026-09-23 S2 新排除：预约单接单到呼叫可能隔十几小时，报价保鲜没有意义，呼叫前 `resolveCallProviders` 见快照过期会自己同步现查一次）；`repeatAnnounce` 对预约单锚在 `acceptDueAt`。
...
4. 商品被软删或规格被删除（不再出现在扫描结果里）→ 状态一并丢弃；**下架不丢状态**，重新上架、库存仍 `< pushBelow` 的规格不再推，只有补货到 `≥pushBelow` 才重置（见上面第 1 条）。
...
### 并发一致性（2026-09-24 修订 2；锁序分析修订 3 改正）

`PUT /api/admin/products/:id/stock` 有规格分支采用「锁读旧值 + 相对增量」而不是「改一个规格 → aggregate 全部规格 → 绝对覆盖商品总库存」——后者在 MySQL REPEATABLE-READ 隔离级别下会用事务开始时的快照覆盖并发下单/取消回滚已经写入的其它规格库存，造成丢更新。改法与取消/退款回滚同一套锁序（规格行 → 商品行）；**2026-09-24 起下单事务改为先扣减再插行，与本接口同序**，插行时外键要的商品行 S 锁被自己已持有的 X 锁覆盖，不再与本接口互锁（加购同批一并对齐，见 `routes/cart.ts`）。本接口保留的死锁重试（最多 5 次，`P2034`/`P2010`）是为「后台整包编辑商品」等锁序尚未对齐的路径兜底，74.11 系列用 raw SQL 模拟旧形态验证的正是这条重试路径仍然有效。详见 3.3 节 `PUT /:id/stock` 的「并发语义」小节与 `docs/superpowers/plans/2026-09-24-order-deadlock.md`；e2e 覆盖见 `scripts/e2e.d/74-low-stock.sh` 的 74.11（含外键 S 锁形态 e/f 两步）与 `scripts/e2e.d/75-order-deadlock.sh`（下单侧同商品并发场景）。
