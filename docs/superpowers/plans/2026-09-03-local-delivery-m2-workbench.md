...
- 顶栏（§8）：打印机状态占位「未接入」（M2b 接上）；日夜切换写 `data-theme`+localStorage（try/catch）；退出确认写明「工作台仍在后台接单、出票和播报」。
- **全屏与专注模式是两件事，别合成一个状态**：`requestFullscreen()` 成功就只是全屏，图例条按 §3 要求**常驻**；只有被浏览器/iframe **拒绝**时才退化为专注模式（隐藏图例与说明）。成功分支也置 `focus=true` 会让店员按一次全屏就永久失去三重编码里的图例。另需监听 `fullscreenchange`，用户按 Esc 原生退出时同步复位。
- **「红」不能一色二义**（§3）：渠道色只用于渠道编码。电话链接、金额等元素若固定用 `--local`，邮寄单上就会出现同城红。随卡片渠道取色。
