...
| P6 | 快递100 已开户、认证、充值、`KD100_KEY/SECRET` 已配置（spec §11 用户侧待办 #1） | ⬜ **待证据核验（不以口头为准）**；`4b73537` 口头 ✅ 已撤回。仓内/本地无开户·充值·密钥证据；Task 6 前须在服务器「系统状态」确认 `kd100.keySet/secretSet` 绿 | 若未完成，Task 6 全程阻塞，其余文档类任务（Task 7–11）可以先做 |
| P7 | 公众平台位置接口权限、隐私保护指引已生效（`docs/miniapp-release-checklist.md` 2.4 已勾选） | ✅ 已确认（checklist 第 71/79/80 行） | — |
| P8 | 「配送通知」（310 配送中）订阅模板已获批并在生产可用 | ⬜ **待证据核验（不以口头为准）**；`4b73537` 口头 ✅ 已撤回。`.env.example` 有选用备注与模板 ID，但 checklist §2.6 未勾配送项、生产可用性未验；Task 6 Step 5 真机收推送再确认 | 若未获批，Task 6 里验证 310 推送这一步会失败，需先处理，不影响其余任务 |
