...
| P4 | **M1 是否已落 `POST /member/points/redeem`、`POST /member/coupons/claim`、`GET /member/mall`、`GET /member/campaign` 四个端点** | ✔ **已回填 M1**（2026-09-05）：M1 Task 7 新增 Step 1b，redeem / claim 两个写端点由 M1 交付；mall / campaign 仍归本计划。原歧义（Task 7 只列三个只读端点，Task 8 的 e2e 却要打发券路径）已在 M1 侧修掉。 | 执行 M2 前仍 `grep -n "router.post" apps/server/src/routes/member.ts` 核实一次；若 M1 执行时漏做，**Task 3 Step 4** 的补齐分支保留作兜底 |
| P5 | `consumePoints` 能告诉调用方「这次扣了哪些入账行、最早到期日是多少」 | ✔ **已回填 M1**（2026-09-05）：M1 Task 3 Step 2 现明确要求返回 `{ minExpiresAt: Date \| null }`，并同步改了 schema 里 `expiresAt` 的列注释。 | spec §5.5 的 `GIFT_REVERT.expiresAt = 被扣行最早到期日` 由 M1 直接满足；**Task 6 Step 1** 的补返回值步骤降级为「核实已有，无则补」的兜底 |
...
- **D3 赠品 `issuedCount` 在全额退款后是否回落 → 不回落。** 一次兑换永久占一个名额。
  后果要写进后台文案：「限量 N 份」是**发放上限**不是**送达上限**，退款单同样占额。
  `PointsGood` 的后台编辑页需注明这一点，避免店员误以为退一单就能多送一份。
