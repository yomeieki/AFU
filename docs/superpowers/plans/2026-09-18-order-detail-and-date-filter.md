...
## 统筹裁定（第三修补轮 R20 上报后，2026-09-18）

**上报事实**（01d 执行方实测，未提交任何改动）：R20 逐字实现后，1024/1100/1180/1279 四档 `fits` 等判据全部通过；**1280×800 下行高回归**——邮寄 PAID 5 按钮行 69→93、SHIP 行 73→93、REFD 行 88→93，超出「与改前同夹具差 ≤2px」。根因：R20 为「配送·取餐」td 加的 `whitespace-nowrap` 在 ≥1280 同样生效，自动布局把富余宽度分给了被 15 位快递单号撑宽的该列（191px），操作列只剩 142px，5 个按钮挤成 3 行。

**裁定：R20 中「配送·取餐 td 加 `whitespace-nowrap`」一项收窄为只在 xl 以下生效，写作 `whitespace-nowrap xl:whitespace-normal`。**
- 理由：03c 加 nowrap 的目的是防止 1024–1279 下该列被弹性商品列挤成一字一行；≥1280 有单独操作列、改前（`f194400`）该列就不禁止折行且行高达标。按断点收窄即可同时满足两档，且不触犯 R20 硬约束（不加 min-w / w-[…]、不缩按钮、不挪断点）。
- R20 其余内容与全部验证判据不变；本裁定不新增、不放宽任何判据。若收窄后 ≥1280 行高仍差 >2px，或 1024–1279 任一档 `fits` / 配送·取餐 `oneLine` 不成立，照旧**停下上报**。

## 统筹裁定（第三修补轮 R20 第二次上报后，2026-09-18）

**上报事实**（01d 实测，未提交）：按上一条裁定收窄 nowrap 后，1024/1100/1180/1279 四档全部通过；1280 下 SHIP002 行 73→93、PAID001 行 69→93（仍 >2px），配送·取餐列 191→154px、操作列 142→166px（内容区 134px），5 按钮仍排 3 行。**对照基线用的是 `37f23ae`**（含已被 02c/03c 判定应撤的 `lg:min-w-[150px]`），且**只实现了 R20、未做 R23**。

**裁定（程序性，判据不放宽）：**
1. **R20–R23 全部实现后再评估 ≥1280 行高**。R23（`lg:max-w-[288px]`，商品列回到 320）本就是 03c 为 ≥lg 列宽设计的一步，是操作列宽度的主要来源；单独评估 R20 不代表最终状态。四条仍各自一个提交，但「≥1280 行高差 ≤2px」这一条判据在 R23 之后统一评估。
2. **对照基线按 03c 原文用 `f194400`**（同夹具、同视口），不用 `37f23ae`——后者含已判定撤除的 `lg:min-w-[150px]`，拿它当基线等于要求在不许用 min-w 的前提下复刻 min-w 的效果。需要在 scratchpad 起一份 `f194400` 的后台（建议 5186）做同夹具对照，结束后删除。
3. **更正我上一条裁定的一处表述**：上一条把配送·取餐的 nowrap 收窄到 xl 以下后，1280 宽下该列允许折行，与 `f194400` 行为一致；因此 R20 验证里「配送·取餐 `oneLine`」只对 1024/1100/1180/1279 四档要求，**1280 档改为「与 `f194400` 同夹具的折行情况一致」**。这是随上一条裁定一起的判据对齐（1280 档回到改前行为），不是放宽；上一条写「判据不增不放宽」时漏写了这一处，特此说明。
4. 若 R20–R23 全部实现后，≥1280 任一行与 `f194400` 同夹具行高差仍 >2px，或 1024–1279 任一档 `fits` / 配送·取餐 `oneLine` 不成立：**停下上报**，不得自行加宽、缩按钮、挪断点或再动商品列上限。

## 统筹裁定（02d 复核后，2026-09-18）

**复核事实**：02d 在 1280×800 邮寄页测到 PAID 5 按钮行 93（`f194400` 69）、SHIPPED 行 93（基线 73）；1290 及以上各宽度与基线一致，回归只落在约 1280–1289。根因：R21 给「售后待处理」小标加的 `whitespace-nowrap` 在 ≥1280 也生效，状态列最小宽 119→126，操作列内容区剩 141px，第二行「发赔偿券 + 重打小票」需要 142px，差 1px。01d 执行方量到 69、02d 量到 93 的分歧与此相符：页面是否出现竖向滚动条（约 15px）决定这 1px 够不够，说明该处余量为零、对内容和环境极敏感。

**裁定：**
1. **新增 R24**：`OrderListTable.tsx:59` 售后小标 `cls` 的 `whitespace-nowrap` 收窄为 `whitespace-nowrap xl:whitespace-normal`——与「配送·取餐 nowrap 收窄到 xl 以下」同一思路：<1280 防止被挤成「售后待 / 处理」（R21 目的不变），≥1280 回到改前行为。02d 已在浏览器临时改样式验证：1280 下小标仍只占一行（高 20），行高回到 69/73/88/73。
2. **R24 验证必须覆盖两种滚动条状态**：1280 与 1285 宽下，分别在「页面有竖向滚动条」与「无竖向滚动条」（可用列表行数或视口高度控制）时，邮寄与同城各行行高与 `f194400` 同条件差 ≤2px；并复测 768/1023/1024/1279 小标 `oneLine`、`fits` 与 B8 至少一处实点。
3. **判据写法对齐（02d 建议 1–3，均改为与基线同条件比较，不降低要求）**：
   - R20「1024 同城 ⑤ ≤ 82」改为「≤ `f194400` 同宽同夹具 +2」（R22 预期内备注掉到单号下一行，基线同为 88）。
   - R20「1024 同城配送·取餐列宽 ≥100」改为「该列各行 `oneLine`」（加 nowrap 后列宽等于最长内容，宽度数字不再有意义；真正要防的是一字一行）。
   - R23「商品 td clientWidth === 320」改为「商品内层 `maxWidth === '288px'`」（无竖向滚动条时卡片更宽，剩余宽度按比例分配，td 宽随之变化，`f194400` 同样如此）。
4. 若 R24 后任一条件下 ≥1280 行高差仍 >2px：**停下上报**，不得加 min-w、缩按钮或内边距、改 gap、挪断点、动商品列上限。
