...
**01 执行 · sonnet（返工轮），2026-09-17：**

- **A14 勘误（尾注模型名判据需改写，非违规）**：本文件 A14 写死 `grep -c "Co-Authored-By: Claude Fable 5.1"`，但署名由执行环境按当前工序角色统一下发，与规划时写死的具体模型名无关（本批各提交实际下发的是 `Claude Opus 5`）。属规划笔误，判据应改按 `grep -c "Co-Authored-By: Claude "` 等于提交数来核（`git log a92d4a3..HEAD --format=%B | grep -c "Co-Authored-By: Claude "` = 9，`git log a92d4a3..HEAD --oneline | wc -l` = 9，相等）。
- **A12/A13/A14 基线勘误（改用 `a92d4a3..HEAD`）**：`9753a3e` 之后到 `a92d4a3` 之间还有两个纯文档提交（`b76898f`、`a92d4a3` 本身），两者新增/改了 `docs/superpowers/plans/2026-09-17-category-anchor.md`、`docs/superpowers/specs/2026-09-17-category-anchor-design.md`、`docs/superpowers/specs/2026-09-17-category-product-sort-design.md` 共 3 个文档文件——这是另一个并行窗口（分类联动那批）的产物，不在本计划白名单内，也从未被本批改动过。用 `9753a3e..HEAD` 起比对会把这 3 个文件误计入「超出白名单」，改用 `a92d4a3..HEAD`（本批实际开工时的分支尖端）核对，`git diff --name-only a92d4a3..HEAD` 逐行核对后均在白名单内。
- **A13 判据写法勘误**：`git diff 9753a3e..HEAD -- apps/server/src/routes/admin/products.ts | grep -c "..."` 默认 3 行上下文口径会把改动附近未改动的上下文行也计入，属判据写法问题而非违规；A13 应固定为 `git diff -U0`（零上下文）只看真正增删的行，此前 01 执行报告已按此口径复核为 0 命中。
- **B1 勘误（含推荐菜的分类需改读法）**：B1「把第三道拖到第一」的期望现象在含推荐菜的分类里应读作「拖到第一个非置顶行的位置」——推荐菜恒排最前、不参与拖拽，"第一"这个位置被置顶区占用时，实际能拖到的最靠前位置就是紧跟在置顶区之后的第一个非置顶行；`curl` 校验时也应确认推荐菜仍在整个列表最前、紧随其后的才是刚拖动的这道菜。
