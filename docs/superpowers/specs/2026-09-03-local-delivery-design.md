...
**建议 D9（待用户确认）**：首期**不做骑手实时位置地图**。订单详情用文字进度 + 骑手姓名/电话即可（3–5 km、20 分钟的配送，顾客关心的是"取餐了没""还有多久到"，而非地图上的点）。若采纳，§5.9 的 `GET /orders/:id/courier` 与 §6 的 `<map>` 组件、30s 轮询移出首期列入二期——零 key、零费用、零合规争议，M3 少一个页面与一个轮询接口。**未确认前 §3.1.7 / §5.9 / §6 的地图相关条目仍按原方案保留。**
...
1. 快递100 开户/认证/充值，key/secret 走 `scripts/set-env.sh`（不截图、不聊天发送）。
