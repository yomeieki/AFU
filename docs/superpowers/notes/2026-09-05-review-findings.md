...
2. **默认 `enabled: false`** —— ✅ **PO 已于 2026-09-05 认可，不是待定项，直接照做。**
   理由：顾客端五个页面要到 M4 才有，现在发分顾客完全看不见；且积分价尚未按 rate=100 重定，
   此时放分等于让顾客攒兑换价随时会变的东西。由店主在 M3 设置页显式打开。
   （与 printer 的默认取向一致。）

   **注意两者不是一回事**：`earnRatePerYuan` 改成 100 是**纠错**（代码与 PO 定稿不符），
   `enabled` 改成 false 是**产品决定**。即使以后打开开关，比例也必须是 100。
...
## ✅ 实验 E1 已完成（2026-09-05，真机 SN 222601993）—— 结果是「乙」

PO 拔掉打印机电源（4G+WiFi 双通道，只有断电才能真离线），实测：

```
拔电后 31s   Open_queryPrinterStatus → "离线。"
离线状态下   Open_printMsg           → ret=0，成功，回执号 222601993_20260905183819_175790234
             Open_queryOrderState    → false（未打印）
             Open_printerInfo        → waiting=1   ← 票排在飞鹅云端
通电后 74s   Open_queryPrinterStatus → "在线，工作状态正常。"
             Open_queryOrderState    → true         ← 那张票自己吐出来了
             Open_printerInfo        → waiting=0
```

**这一结果改写 H5 与 H5b，两条都必须按下面的事实重修，不得按原文字面修：**

1. **`print()` 在打印机离线时不会失败**。所以 `PrintJob` 永远走不到 `FAILED`，
   H5 描述的「重试 3 次 → FAILED → 恢复后补打」**在真实世界里不会发生**——
   按 H5 字面去修，是给一段永远跑不到的代码打补丁。**H5 的修法作废。**
2. **票堆在飞鹅云端，不在我们的表里**。`waiting` 字段可观测到积压。
   我们无法挑选补打，因为根本不是我们在补——是飞鹅在恢复瞬间**全部自动吐出**。
3. **规格「超过 30 分钟的旧单不再补打」只能靠 `Open_delPrinterSqs`**，而它全仓未实现（H5b）。
   注意该接口是**清空整个队列**，不能按单删。所以正确做法是：
   检测到恢复且 `waiting > 0` 时 → 调 `Open_delPrinterSqs` 清空 → 再从我们自己的 `PrintJob` 表里
   把 30 分钟内、尚未确认 PRINTED 的重新下发。我们表里有全部记录，这条路走得通。

**真正要做的事（取代 H5 原修法）**：
- 下发前/下发后查在线状态，离线时 `PrintJob` 记 SENT + 标记「已入云端队列」，而不是当作失败重试
- `printerHealth` 检测到「从离线变在线」且 `waiting > 0` 时，执行上面第 3 条的清队列 + 择优重发
- `Printer` 接口新增 `clearQueue(sn)` 方法，`feie.ts` 实现 `Open_delPrinterSqs`，`mock.ts` 同步

---

### H5b · `Open_delPrinterSqs` 全仓未实现 —— 规格要求的「不补打 30 分钟前旧单」做不到

**2026-09-05 追加，不是复核方给的，是排断网实验时查出来的。**

**证据（不需要真机就能确立）**：
- `Open_delPrinterSqs` 官方说明是「清空该打印机待打印队列」（调研文档 §1 接口表）
- `Open_printerInfo` 返回 `waiting` 字段（待打印数），飞鹅后台也显示「待打印 N」
- → **飞鹅有云端队列**，票下发成功后是排在飞鹅那边，不是排在我们的 `PrintJob` 表里

规格 §8b 的接口清单里明确写了 `Open_delPrinterSqs`（**清队列，离线恢复前用于丢弃 30 分钟前的旧单**），
说明规格作者当初就知道这件事。但 `grep -rn delPrinterSqs apps/server/src/` **零命中**，
`services/ticket/printer.ts` 的 `Printer` 接口只声明了 `print/queryStatus/queryJob/bindPrinter`，
**连方法都没有**。

**后果**：规格「离线期间新单记 PENDING，恢复后自动补打（超过 30 分钟的旧单不再补打，避免一次吐一堆）」
的后半句无法执行。我们的代码以为票堵在自己表里所以能挑着补，实际上票已经交给飞鹅排队，
打印机一上线**全部一起吐**，我们无从干预。夜间离线一晚，早上开机吐一地。

**与 H5 的关系**：同一个病根。H5 说的是「我们这边该补的没补」，H5b 说的是
「飞鹅那边不该吐的会吐」。**两条必须一起修，且都依赖下面那个实验的结果**——
如果 `Open_printMsg` 在打印机离线时返回 `ret=0`（云端排队），那么 `PrintJob` 根本不会走到
FAILED，H5 描述的整条「重试→FAILED→恢复补打」在真实世界里压根不会发生，
真正要做的是**下发前先查在线状态 + 恢复时先调 `Open_delPrinterSqs` 清掉旧的**。
...
1. ~~会员积分默认开关~~ —— ✅ **已决**（2026-09-05，PO 认可默认关）。详见 B3。
2. ~~实验 E1~~ —— ✅ **已完成（2026-09-05），结果是「乙」，详见上方 H5 前的实验记录。**
   ~~⚠️ 原设计有两处错误，已修正~~：
   ① 该机型是 FP-V58-**WHC**，4G + WiFi 双通道，**拔网线它会自己切到 4G**，不构成离线。
   **唯一可靠的断网方式是拔电源。**
   ② 原写「后台点打印测试页」，但**后台打印机设置页要到 M3 才有**，现在只有接口没有界面。
   改由本会话用 scratchpad 脚本直接调接口。

   修正后的流程：PO 拔掉打印机电源 → 我用只读接口确认飞鹅那边已识别为「离线。」
   → 我下发一张测试票，记录返回的 ret 码 → PO 通电 → 观察是否自动吐纸。
   ~~断开打印机网络 → 后台点「打印测试页」→~~
