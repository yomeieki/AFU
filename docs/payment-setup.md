...
| 后台退款报 `ORDER_NOT_EXIST` / `PARAM_ERROR` | 订单是模拟支付或 out_trade_no 缺失 | 生产不该出现；出现即按运维流程改库（「手动标记完成」2026-09-22 已删） |
| 退款一直「微信处理中」 | 退款回调没到(nginx 未放行 `/api/wechat/pay/` 或验签失败) | 查日志 `[wechat-refund-notify]`;自动补查每 5 分钟查微信,查到成功自动落账;半小时未果告警群提醒 |
...
- **一键自动退款(已实现 ✅ 2026-09)**:
  ...
  - 退款结果由微信退款回调自动写状态(退款中 → 已退款),失败/异常自动告警到企微群;回调丢失由自动补查(2026-09-21)兜底,「手动标记完成」已于 2026-09-22 去掉
