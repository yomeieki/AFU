...
if [[ -n "$D44_DUAL_OID" ]]; then
  ...
  # ⚠️ 同电话那条的教训：**断言内容本身，不要断言它旁边的标签文字**。
  # 「地址：」→「地址 」这类文案微调（放大后冒号那一格会把行挤爆）会让标签式断言当场空过。
  D44_DETAIL=$(sql "SELECT receiver_detail FROM orders WHERE id=$D44_DUAL_OID;")
  [[ -n "$D44_DETAIL" && "$D44_DELIV" == *"$D44_DETAIL"* ]] \
    && ok "配送联：有地址详址（骑手要用，按内容断言：$D44_DETAIL）" \
    || fail "配送联：缺地址" "${D44_DELIV:0:200}"
  # 手机尾号（PO 2026-09-08）：骑手 app 上是运力方的单号，跟 #后四位对不上；它一定显示的是
  # 收件人手机，所以配送联把尾号放大到单号同一行。厨房联**不能**有——后厨不该看到联系方式。
  D44_PHONE=$(sql "SELECT receiver_phone FROM orders WHERE id=$D44_DUAL_OID;")
  [[ -n "$D44_PHONE" && "$D44_DELIV" == *"<CB>尾号${D44_PHONE: -4}</CB>"* ]] \
    && ok "配送联：票头是放大的手机尾号（尾号${D44_PHONE: -4}），不带单号" \
    || fail "配送联：票头尾号不对" "${D44_DELIV:0:200}"
  # PO 2026-09-09 定：票面一律不印单号，认单只用尾号——footer 那行「单号：#xxxx」已删，这里断言别漏改回去
  [[ "$D44_DELIV" != *"单号：#"* ]] \
    && ok "配送联：不含「单号：#」（票面不再印单号）" \
    || fail "配送联：仍含「单号：#」" "${D44_DELIV:0:200}"
  # 厨房联同样印尾号（PO 2026-09-08 定）：两联靠同一个数联系，装袋时对得上
  [[ "$D44_KITCH" == *"<CB>尾号${D44_PHONE: -4}</CB>"* ]] && ok "厨房联：同一个尾号（两联能对上）" || fail "厨房联缺尾号" "${D44_KITCH:0:200}"
  # 从库里取实付金额、换算成票面写法再比。不要拿票面自己 grep 出来的串回头去票面里找——
  # 那种写法恒真（grep 空时 `*""*` 还匹配一切），是假断言。
  D44_PAID_FEN=$(sql "SELECT actual_amount FROM orders WHERE id=$D44_DUAL_OID;")
  D44_PAID_YUAN=$(awk -v f="${D44_PAID_FEN:-0}" 'BEGIN{printf "%.2f", f/100}')
  [[ "$D44_DELIV" == *"实付：¥${D44_PAID_YUAN}"* ]] \
    && ok "配送联：实付金额与订单一致（¥${D44_PAID_YUAN}）" \
    || fail "配送联：实付金额缺失或与订单对不上（期望 ¥${D44_PAID_YUAN}）" "${D44_DELIV:0:200}"
  [[ -n "$D44_DETAIL" && "$D44_KITCH" != *"$D44_DETAIL"* ]] \
    && ok "厨房联：**不含地址**（顾客住址不进后厨，按内容断言）" \
    || fail "厨房联：仍含地址" "$D44_KITCH"
  [[ "$D44_KITCH" != *"实付"* ]] && ok "厨房联：**不含金额**" || fail "厨房联：仍含金额" "$D44_KITCH"
  # 姓名与地址必须是放大的（<B>），电话必须不是——PO 2026-09-06 定的字号分工
  [[ "$D44_DELIV" == *"<B>收货人 "* ]] && ok "配送联：收货人放大（<B>）" || fail "配送联：收货人没放大" "${D44_DELIV:0:200}"
  [[ "$D44_DELIV" == *"<B>地址 "* ]] && ok "配送联：地址放大（<B>）" || fail "配送联：地址没放大" "${D44_DELIV:0:200}"
  [[ "$D44_DELIV" != *"<B>电话 "* ]] && ok "配送联：电话**不**放大（已脱敏，放大也拨不出去）" || fail "配送联：电话被放大了" "${D44_DELIV:0:200}"
