...
# S7（2026-09-23 修，批次二 D3）：真机 <B> 只有 16 列可用宽度，「取餐 日期 时段」整段进 <B>
# 会超宽折行。改成普通字号日期行 + 独立的放大时段行，这里的字面匹配同步微调到新格式，
# 验证的仍是同一件事——票面印了取餐日期与放大的取餐时段。
[[ "$P62_T" == *"取餐 "* ]] && echo "$P62_T" | grep -qE '<B>[0-9]{2}:[0-9]{2}–[0-9]{2}:[0-9]{2}</B>' && ok "印取餐时间（S7：日期普通行 + 放大时段行）" || fail "没印取餐时间" "$P62_T"
